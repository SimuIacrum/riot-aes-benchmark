...
static const uint8_t tag_len = 1;
...
void executeAesOcb(int numberOfRounds, int keySize, size_t messageLength, size_t authDataLength){
    for(int round = 1; round <= numberOfRounds; round++){
        ...
        cipher_encrypt_ocb(&cipher, auth_data, authDataLength, tag_len, iv, sizeof(iv), input, total, output);
        ...
        cipher_decrypt_ocb(&cipher, auth_data, authDataLength, tag_len, iv, sizeof(iv), output, total, decryptBuffer);
